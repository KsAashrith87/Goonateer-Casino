...
function safe(room,viewerId){
  ...
  const blackjackSafe = room.blackjack ? {
    ...room.blackjack,
    // Dealer hole card stays face down while the round is active.
    dealer: room.blackjack.active
      ? [{hidden:true}, ...(room.blackjack.dealer||[]).slice(1)]
      : (room.blackjack.dealer||[])
  } : null;
  return {
    ...room,
    blackjack: blackjackSafe,
    players:room.players.map(p=>({
      ...p,
      // Blackjack player cards are visible, poker hole cards are private.
...
function bjCurrentPlayer(room){
  const bj=room.blackjack; if(!bj)return null;
  return room.players[bj.turn] || null;
}
function bjAdvance(room){
  const bj=room.blackjack; if(!bj?.active)return;
  for(let i=0;i<room.players.length;i++){
    bj.turn++;
    if(bj.turn>=room.players.length){ bjFinish(room); return; }
    const p=room.players[bj.turn];
    if(p && p.status==='Playing'){
      bj.message=`${p.name}'s turn`;
      return;
    }
  }
  bjFinish(room);
}
function bjStart(room){
  let d=deck();
  room.blackjack={deck:d,dealer:[d.pop(),d.pop()],turn:0,active:true,message:''};
  ...
  room.blackjack.message=room.players[0] ? `${room.players[0].name}'s turn` : 'Blackjack started';
}
function bjFinish(room){
  const bj=room.blackjack;const dealer=bj.dealer;
  while(total(dealer)<17)dealer.push(bj.deck.pop());
  const dt=total(dealer);
  for(const p of room.players){
    const pt=total(p.hand);
    if(pt<=21&&(dt>21||pt>dt)){p.chips+=p.bet*2;p.status='Won'}
    else if(pt===dt&&pt<=21){p.chips+=p.bet;p.status='Push'}
    else p.status='Lost';
    p.bet=0;
  }
  bj.active=false;
  bj.message='Round complete';
}
...
socket.on('bjStart',()=>{const room=rooms.get(socket.data.room);if(!room)return;bjStart(room);emitRoom(room.code)});
socket.on('bjHit',()=>{const room=rooms.get(socket.data.room),p=room?.players.find(x=>x.id===socket.id);if(!room?.blackjack?.active||!p)return;const current=bjCurrentPlayer(room);if(!current||current.id!==socket.id)return;p.hand.push(room.blackjack.deck.pop());if(total(p.hand)>21){p.status='Bust';bjAdvance(room)}else{room.blackjack.message=`${p.name}'s turn`;}emitRoom(room.code)});
socket.on('bjStand',()=>{const room=rooms.get(socket.data.room),p=room?.players.find(x=>x.id===socket.id);if(!room?.blackjack?.active||!p)return;const current=bjCurrentPlayer(room);if(!current||current.id!==socket.id)return;p.status='Stand';bjAdvance(room);emitRoom(room.code)});
